...
export async function POST(request: NextRequest) {
  try {
    ...
    const todayLogs = recentLogs.filter(log => 
      new Date(log.scheduledFor).toISOString().split('T')[0] === today
    )
    ...
    const notificationData = {
      ...
      recentMedications: todayLogs.map(log => ({
        ...
        status: log.status,
        takenAt: log.takenAt ? new Date(log.takenAt).toLocaleTimeString() : null
      })),
      urgency,
      type,
    ...
    for (const caregiver of senior.caregivers) {
      ...
      try {
        ...
        if (familyMember.email) {
          const emailSent = await sendEmailNotification(familyMember.email, notificationData)
          notifications.push({
        ...
        if (familyMember.phone) {
          const smsSent = await sendSMSNotification(familyMember.phone, notificationData)
          notifications.push({
            recipientId: familyMember.id,
            recipientName: familyMember.name,
            method: 'sms',
        ...
        console.error(`Failed to send notification to ${familyMember.name}:`, error)
        notifications.push({
          recipientId: familyMember.id,
          recipientName: familyMember.name,
          method: 'email',
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!seniorId) {
      return NextResponse.json({ error: 'Senior ID required' }, { status: 400 })
    }
...
async function sendEmailNotification(email: string, data: any): Promise<boolean> {
...
function formatEmailContent(data: any): string {
  const statusEmoji = data.adherencePercentage >= 90 ? '🟢' : 
  ...
  return `
    ...
    Medications Taken Today: ${data.medicationsTakenToday}/${data.totalMedicationsToday}
    
    Today's Medication Summary:
    ${data.recentMedications.map((med: any) => 
      `• ${med.name} (${med.dosage}) at ${med.scheduledTime} - ${
        med.status === 'TAKEN' ? '✅ Taken' + (med.takenAt ? ` at ${med.takenAt}` : '') : 
...
function formatSMSContent(data: any): string {
  ...
  return `${statusEmoji} ${data.seniorName} Health Update: ${data.adherencePercentage}% medication adherence today (${data.medicationsTakenToday}/${data.totalMedicationsToday} taken). ${data.urgency === 'high' ? 'Needs attention.' : 'All good!'} - MediCare Assistant`
}
...
function generateMockNotificationHistory(seniorId: string, days: number) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    notifications.push({
      ...
      recipients: [
        { name: 'Priya Sharma', email: 'priya@email.com', method: 'email', status: 'sent' },
        { name: 'Dr. Sarah Johnson', email: 'sarah@familydoc.com', method: 'email', status: 'sent' }
      ],
      summary: {
        adherencePercentage: adherence,
        medicationsTaken: adherence >= 90 ? 3 : adherence >= 70 ? 2 : 1,
        totalMedications: 3,
        status: adherence >= 90 ? 'excellent' : adherence >= 70 ? 'good' : 'needs_attention'
      }
    })
  }

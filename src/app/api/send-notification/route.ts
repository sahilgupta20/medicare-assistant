...
export async function POST(request: NextRequest) {
  try {
    const { familyMember, message, urgency, medicationDetails } = await request.json();
    
    console.log(`Sending Gmail notification to ${familyMember.name} at ${familyMember.email}`);
    
    const emailResult = await emailTransporter.sendMail({
      from: `"MediCare Assistant" <${process.env.EMAIL_USER!}>`,
      to: familyMember.email,
      subject: urgency === 'high' ? 'URGENT - MediCare Alert' : 'MediCare Notification',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 20px; background: white;">
            <h2 style="color: ${urgency === 'high' ? '#dc2626' : '#f59e0b'};">
              ${urgency === 'high' ? '🚨 URGENT ALERT' : '💊 Medication Reminder'}
            </h2>

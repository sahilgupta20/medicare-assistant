  ...
  const performLogin = async (email: string, password: string) => {
    ...
    try {
      ...
      if (result?.ok) {
        ...
        // The middleware will handle the proper redirect based on role
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white py-3 px-4 rounded-xl font-semibold transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
            >
              {loading ? (
                <>
                  <Loader2 className="h-5 w-5 animate-spin mr-2" />
            ...
            <div className="grid grid-cols-1 gap-2">
              {demoUsers.slice(0, 3).map((user) => (
                <button
                  ...
                  onClick={() => handleDemoLogin(user)}
                  disabled={loading}
                  className="w-full text-left p-3 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <div className="font-medium text-gray-900">{user.name}</div>
                  <div className="text-gray-500">{user.email}</div>
                </button>

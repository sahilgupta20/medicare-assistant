...
export default function FamilyPage() {
        ...
        const todayMedications = medications.flatMap((med: any) =>
          med.times.split(",").map((time: string) => {
            ...
            const todayLog = logs.find((log: any) => {
              ...
              const logTime = new Date(log.scheduledFor)
                .toTimeString()
                .slice(0, 5);
              return (
                logDate === today &&
  ...
  const handleAlertAction = async (
    alertId: string,
    action: "acknowledge" | "call" | "escalate"
  ) => {
    try {
      ...
      if (response.ok) {
        ...
        if (action === "acknowledge") {
          ...
          alert(
            `Alert resolved successfully. The family has acknowledged that the medication issue has been addressed.`
          );
        } else if (action === "call") {
  ...
  const todayMissed = todayStatus.filter(
          ...
          <div className="max-w-7xl mx-auto px-6 py-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-6">
                <Link
                  href="/medications"
                  className="p-3 hover:bg-blue-100 rounded-2xl transition-all duration-300 group"
                >
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 text-lg">
                      Stay connected with {seniorName}'s health
                    </p>
              ...
              <div className="flex items-center space-x-4">
                ...
                {/* Send Update - Only FAMILY and ADMIN can send updates */}
                ...
                {user?.role === "DOCTOR" && (
                  <div className="bg-gray-100 text-gray-600 px-6 py-3 rounded-2xl text-lg font-semibold flex items-center space-x-2">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-sm opacity-90">
                          Scheduled for {alert.scheduledTime} • Missed{" "}
                          {alert.missedTime}
                        </p>
                      </div>

                      {/* Action buttons - only FAMILY, ADMIN, and CAREGIVER can take action */}
                      {(user?.role === "FAMILY" ||
                        user?.role === "ADMIN" ||
                        user?.role === "CAREGIVER") && (
                      ...
                      {/* View-only message for restricted roles */}
                      {user?.role === "DOCTOR" && (
                        <div className="text-gray-500 text-sm font-medium">
                          Medical oversight - Contact primary caregiver to
                          resolve
                        </div>
                      )}
                    </div>
            ...
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-lg">
                    {todayMissed === 0
                      ? `${seniorName} is doing great today!`
                      : todayMissed === 1
                      ? `${seniorName} missed 1 medication today`
              ...
              <div className="text-right">
                <div
                  className={`text-5xl font-bold ${
                    todayMissed === 0 ? "text-green-600" : "text-red-600"
                  }`}
                >
          ...
          <div className="bg-gradient-to-br from-white via-rose-50/30 to-pink-50/50 rounded-[2.5rem] shadow-2xl p-10 border border-white/50">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {familyMembers.map((member) => (
                <div
                  key={member.id}
                  className="bg-white/80 backdrop-blur-sm border-2 border-rose-200 rounded-3xl p-8 hover:shadow-xl transition-all duration-500 group"
                >

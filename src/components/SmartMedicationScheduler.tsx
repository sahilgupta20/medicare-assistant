...
import { Calendar, Clock, Plus, Edit, Trash2, Bell, AlertCircle, CheckCircle2, Coffee, Sun, Sunset, Moon } from 'lucide-react'
...
interface MedicationSchedule {
  ...
  dosage: string
  time: string
  frequency: 'daily' | 'weekly' | 'as_needed' | 'twice_daily' | 'three_times_daily'
  dayOfWeek?: number // 0-6, for weekly medications
  isActive: boolean
...
export default function SmartMedicationScheduler({ medications, onScheduleUpdate }: SmartSchedulerProps) {
  const [schedules, setSchedules] = useState<MedicationSchedule[]>([])
  const [showAddModal, setShowAddModal] = useState(false)
  const [editingSchedule, setEditingSchedule] = useState<MedicationSchedule | null>(null)
  const [selectedMedication, setSelectedMedication] = useState('')
  const [schedulingConflicts, setSchedulingConflicts] = useState<Array<{
    time: string
    medications: string[]
  }>>([])
  ...
  // Predefined time suggestions based on common medication schedules
  const timePresets = {
  ...
  const getTimeIcon = (timeOfDay: string) => {
    switch (timeOfDay) {
      case 'morning': return <Coffee className="h-5 w-5 text-amber-600" />
      case 'afternoon': return <Sun className="h-5 w-5 text-yellow-600" />
      case 'evening': return <Sunset className="h-5 w-5 text-orange-600" />
      case 'night': return <Moon className="h-5 w-5 text-indigo-600" />
      ...
      {schedulingConflicts.length > 0 && (
        <div className="bg-amber-50 border-2 border-amber-200 rounded-2xl p-6 mb-8">
          <div className="flex items-center space-x-3 mb-3">
            ...
            <h3 className="text-xl font-bold text-amber-800">Scheduling Conflicts</h3>
          </div>
          {schedulingConflicts.map((conflict, index) => (
            <div key={index} className="text-amber-700">
              <strong>{conflict.time}:</strong> {conflict.medications.join(', ')} 
      ...
      {generateSmartSuggestions().length > 0 && (
        <div className="bg-blue-50 border-2 border-blue-200 rounded-2xl p-6 mb-8">
          <div className="flex items-center space-x-3 mb-3">
            ...
            <h3 className="text-xl font-bold text-blue-800">Smart Suggestions</h3>
          </div>
          <div className="text-blue-700">
            <p className="mb-2">Consider adding medications at these optimal times:</p>
        ...
        {Object.entries(groupedSchedules).map(([timeOfDay, schedules]) => (
          <div key={timeOfDay} className="bg-gray-50 rounded-2xl p-6">
            <div className="flex items-center space-x-3 mb-6">
              ...
              <h3 className="text-2xl font-bold text-gray-800 capitalize">{timeOfDay}</h3>
              <span className="text-gray-500">({schedules.length} medications)</span>
            </div>
              ...
              {schedules
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="text-2xl font-bold text-gray-800">{schedule.time}</div>
                  ...
                  <div className="space-y-2">
                    <div className="font-semibold text-gray-800">{schedule.medicationName}</div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setNewSchedule(prev => ({ ...prev, reminderMinutesBefore: parseInt(e.target.value) }))}
                  className="w-full p-4 text-lg border-2 border-gray-300 rounded-xl focus:border-blue-500 focus:outline-none"
                >
              ...
              <div>
                <label className="flex items-center space-x-3">
                  <input
                    ...
                    onChange={(e) => setNewSchedule(prev => ({ ...prev, withFood: e.target.checked }))}
                    className="w-5 h-5"
                  />
                  <span className="text-lg font-medium text-gray-700">Take with food</span>
                </label>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setNewSchedule(prev => ({ ...prev, specialInstructions: e.target.value }))}
                  className="w-full p-4 text-lg border-2 border-gray-300 rounded-xl focus:border-blue-500 focus:outline-none"
                  rows={3}

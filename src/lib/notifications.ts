...
class MediCareNotificationService {
  ...
  private initialize() {
    ...
    this.detectTimezone();
    console.log(`🔔 Notification service initialized with timezone: ${this.state.timezone}`);
  }
  ...
  private detectTimezone() {
    try {
      ...
      if (userTimezone === 'Asia/Kolkata' || userTimezone === 'Asia/Calcutta') {
        ...
        console.log(` User timezone (${userTimezone}) differs from IST, using user timezone`);
  ...
  private async showTestNotification(): Promise<void> {
    ...
    try {
      ...
      this.showFallbackAlert(` Test Alert: MediCare notifications working! IST: ${currentIST}`);
      this.playNotificationSound();
    }
  }
  ...
  scheduleReminders(medications: MedicationSchedule[]): void {
    ...
    console.log(` Scheduling ${medications.length} medication reminders for ${this.state.timezone}`);
  ...
  private scheduleSingleReminder(medication: MedicationSchedule): void {
    ...
    try {
      ...
      if (Array.isArray(timeData)) {
        timeStrings = timeData.filter(time => time && typeof time === 'string');
      } else if (typeof timeData === 'string') {
        timeStrings = timeData.split(',').map(t => t.trim()).filter(Boolean);
      } else {
      ...
      console.log(` Found ${timeStrings.length} time slots for ${medication.name}:`, timeStrings);
  ...
  private scheduleTimeSlot(medication: MedicationSchedule, timeString: string): void {
    try {
      ...
      if (isNaN(hours) || isNaN(minutes) || hours < 0 || hours > 23 || minutes < 0 || minutes > 59) {
        console.warn(` Invalid time values for ${medication.name}: ${cleanTime}`);
        return;
      }
      ...
      console.log(` ${medication.name} at ${cleanTime} (${this.state.timezone}): reminder in ${minutesUntil} minutes (${targetTime.toLocaleString('en-IN', { timeZone: this.state.timezone })})`);

      // Schedule the medication reminder
      const timeoutId = window.setTimeout(() => {
        this.showMedicationReminder(medication, cleanTime);
        
        // Schedule missed medication detection
      ...
      console.error(` Failed to schedule time slot ${timeString} for ${medication.name}:`, error);
    }
  }

  // missed medication detection with database integration
  private scheduleMissedMedicationCheck(medication: MedicationSchedule, timeSlot: string, scheduledTime: Date): void {
    const missedCheckDelay = 15 * 60 * 1000; 
    const checkTime = scheduledTime.getTime() + missedCheckDelay;
    const now = Date.now();
    
    if (checkTime <= now) {
      console.log(` Skipping missed check for past time: ${medication.name} at ${timeSlot}`);
      return;
    }

    const timeUntilCheck = checkTime - now;
    console.log(` Scheduling missed check for ${medication.name} at ${timeSlot} in ${Math.round(timeUntilCheck / 1000 / 60)} minutes`);

    const missedTimerId = window.setTimeout(async () => {
      const wasTaken = await this.checkMedicationStatus(medication.id, timeSlot, scheduledTime);
      ...
      if (!wasTaken) {
        ...
        console.log(` Medication confirmed taken: ${medication.name} at ${timeSlot}`);
      }
    }, timeUntilCheck);

    this.state.missedMedicationTimers.set(`missed-${medication.id}-${timeSlot}`, missedTimerId);
  }


  private async checkMedicationStatus(medicationId: string, timeSlot: string, scheduledTime: Date): Promise<boolean> {
    try {
      const dateString = scheduledTime.toISOString().split('T')[0];
      const response = await fetch(`/api/medication-logs?medicationId=${medicationId}&time=${timeSlot}&date=${dateString}`);
      
      if (response.ok) {
        const logs = await response.json();
        const takenLog = logs.find((log: any) => 
          log.status === 'TAKEN' && 
  ...
  async showMedicationReminder(medication: MedicationSchedule, timeSlot?: string): Promise<void> {
    this.initialize();
    ...
    try {
      ...
      console.warn(' Browser notification failed for medication reminder:', error);
  ...
  markMedicationTaken(medicationId: string, timeSlot?: string): void {
    console.log(` Medication ${medicationId} marked as taken${timeSlot ? ' at ' + timeSlot : ''}`);
    
    // Clear missed medication timer
    const missedKey = timeSlot ? `missed-${medicationId}-${timeSlot}` : medicationId;
  ...
  public logScheduleSummary(): void {
    ...
    console.log('=====================================');
    console.log(`Current time (${this.state.timezone}): ${this.getCurrentIST()}`);
    console.log(`Active reminders: ${this.state.scheduledReminders.size}`);
    console.log(`Missed medication timers: ${this.state.missedMedicationTimers.size}`);

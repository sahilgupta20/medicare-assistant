...
class VoiceInterfaceService {
  ...
  private async processCommand(transcript: string) {
    ...
    } else if (command.includes('what medications') || command.includes('what pills') || command.includes('what medicine')) {
      this.handleListMedications();
      
    } else if (command.includes('did i take') || command.includes('have i taken')) {
      const medicationName = this.extractMedicationName(command);
      this.handleCheckTaken(medicationName);
      
    } else if (command.includes('i took') || command.includes('just took') || command.includes('i have taken')) {
      const medicationName = this.extractMedicationName(command);
      this.handleMarkTaken(medicationName);
      
    } else if (command.includes('what time') || command.includes('when should')) {
      const medicationName = this.extractMedicationName(command);
      this.handleMedicationTime(medicationName);
      
    } else if (command.includes('help') || command.includes('what can you do')) {
      this.speak('I can help you track medications, check if you\'ve taken them, and remind you about schedules. Try saying "What medications do I need today?" or "I took my heart medicine".');
      
    } else {
      this.speak('I didn\'t understand that. Try saying "help" to see what I can do, or say "Hey MediCare" to get started.');
    }
  }
  ...
  private async handleCheckTaken(medicationName: string) {
    ...
    if (!medication) {
      if (medicationName) {
        this.speak(`I couldn't find a medication called ${medicationName}. Please check the name and try again.`);
      } else {
    ...
    const status = medication.isTaken ? 'Yes, you took' : 'No, you haven\'t taken';
  ...
  private async handleMarkTaken(medicationName: string) {
    ...
    if (!medication) {
      if (medicationName) {
        this.speak(`I couldn't find a medication called ${medicationName}. Could you repeat the name clearly?`);
      } else {
  ...
  private speak(text: string) {
    if (typeof window === 'undefined' || !this.synthesis) {
      console.log('🗣️ Would speak:', text);
      return;
    }
  ...
  public startListening() {
    ...
    try {
      ...
      this.speak('Sorry, I couldn\'t start listening. Please check your microphone permissions.');
    }
  }
  ...
  public setMedicationTakenCallback(callback: (medicationId: string) => void) {
    ...
    console.log('🔗 Voice interface callback set up successfully');
  }

  public updateMedications(medications: MedicationData[]) {
    this.medications = medications;
    console.log(`🗣️ Updated medication list: ${medications.length} medications`);
  }
...
export function useVoiceInterface() {
  ...
  return {
    ...
    updateMedications: (meds: MedicationData[]) => voiceInterfaceService?.updateMedications(meds),
